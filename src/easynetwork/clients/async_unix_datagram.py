...
class AsyncUnixDatagramClient(AbstractAsyncNetworkClient[_T_SentPacket, _T_ReceivedPacket]):
    ...
    async def __create_endpoint(
        ...
        *,
        protocol: DatagramProtocol[_T_SentPacket, _T_ReceivedPacket],
        unix_socket_to_delete: dict[pathlib.Path, int],
        ...
        if (path := local_name.as_pathname()) is not None:
            unix_socket_to_delete[path] = os.stat(path).st_ino
        ...
        async with contextlib.AsyncExitStack() as stack:
            ...
            try:
                ...
                if unix_socket_to_delete:
                    stack.push_async_callback(self.__backend.run_in_thread, self.__cleanup_unix_socket, unix_socket_to_delete)
    ...
    @staticmethod
    def __cleanup_unix_socket(unix_socket_to_delete: dict[pathlib.Path, int]) -> None:

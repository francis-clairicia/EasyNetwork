...
from ..lowlevel.socket import SocketProxy, UnixCredentials, UnixSocketAddress, UNIXSocketAttribute, get_peer_credentials
...
class AsyncUnixStreamClient(AbstractAsyncNetworkClient[_T_SentPacket, _T_ReceivedPacket]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Socket Parameters:
            socket: An already connected Unix :class:`socket.socket`.
    ...
    def get_peer_credentials(self) -> UnixCredentials:  # noqa: F811  # False positive
        """
        ...
        """
        endpoint = self.__endpoint.get_sync()
        socket = endpoint.extra(UNIXSocketAttribute.socket)
        return self.__peer_creds_cache.get(socket, get_peer_credentials)

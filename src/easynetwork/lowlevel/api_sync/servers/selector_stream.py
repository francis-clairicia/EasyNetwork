...
class SelectorStreamServer(_transports.BaseTransport, Generic[_T_Request, _T_Response]):
    ...
    def __serve_requests__start_new_client(
        ...
        *,
        ...
        with contextlib.ExitStack() as task_exit_stack:
            ...
            request_receiver = self.__new_request_receiver(transport, disconnect_error_filter)
            ...
            return self.__serve_requests__receive_next_request(
                None,
                selector_token=selector_token,
                executor=executor,
                client=client,
    ...
    def __serve_requests__receive_next_request(
        self,
        ...
        *,
        ...
        with contextlib.ExitStack() as on_error_exit_stack:
            task_exit_stack = on_error_exit_stack.enter_context(task_exit_stack.pop_all())
            request_future: concurrent.futures.Future[_T_Request] = _future_factory()
            try:
                if client.is_closing() or (reader_future is not None and reader_future.cancelled()):
                    _cancel_future_and_notify(request_future)
                else:
                    try:
                        if reader_future is None:
                            receive_timeout = _utils.validate_optional_timeout_delay(receive_timeout, positive_check=True)
                        else:
                            _assert_future_is_done(reader_future)
                        ...
                        request: _T_Request
                        try:
                            request = request_receiver.next(first_try=(reader_future is None))
                            ...
                            reader_future = selector_token.register(
                                ...
                                reader_condvar=request_receiver.reader_condvar,
                                reader_done=request_receiver.reader_done,
                            )
                            reader_future.add_done_callback(
                                functools.partial(
                                    self.__serve_requests__receive_next_request,
                                    selector_token=selector_token,
                                    executor=executor,
                                    client=client,
                                    request_receiver=request_receiver,
                            ...
                            on_error_exit_stack.pop_all()
                            return
                        else:
                            request_future.set_result(request)
                    except BaseException as exc:
                        _set_future_exception_unless_cancelled(request_future, exc)

                _assert_future_is_done(request_future)
                try:
                    ...
                    handler_future = _future_factory()
                ...
                del self, reader_future, request_future  # Break reference cycle with request future on error.
    ...
    def __serve_requests__handle_client_request(
        self,
        request_future: concurrent.futures.Future[_T_Request],
        /,
        *,
        selector_token: _ThreadSafeSelectorToken,
        executor: concurrent.futures.Executor,
        ...
        with task_exit_stack.pop_all() as task_exit_stack:
            ...
            request_exc: BaseException | None
            timeout: float | None
            try:
                _assert_future_is_done(request_future)
                if request_future.cancelled():
                    # server is stopping or client is closing.
                    return
                if request_exc := request_future.exception():
                    match request_exc:
                        case StopIteration():
                            # EOF.
                            request_handler_context.run(request_handler_generator.close)
                            raise request_exc
                        case _:
                            timeout = request_handler_context.run(request_handler_generator.throw, request_exc)
                else:
                            ...
                            while True:
                                ...
                                with request_receiver:
                                    if client.is_closing():
                                        return
                                    available, timeout = transport._wait_for_fd(
        ...
        match self.__protocol:
            case BufferedStreamProtocol():
                ...
                return _BufferedRequestReceiver(
                    ...
                    consumer=consumer,
                    disconnect_error_filter=disconnect_error_filter,
                    server_is_shutting_down=self.__is_shut_down.is_set,
                )
            case StreamProtocol():
        ...
        for key, _ in events:
            ...
            match key.data:
                case _SelectorKeyData(future=task_future, start_time=start_time):
                    ...
                    _set_future_result_unless_cancelled(task_future, now - start_time)
...
def _assert_future_is_done(f: concurrent.futures.Future[Any]) -> None:
    if not f.done():
        raise AssertionError("Expected future to be done.")

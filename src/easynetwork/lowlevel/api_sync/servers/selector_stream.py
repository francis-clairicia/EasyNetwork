...
class SelectorStreamServer(_transports.BaseTransport, Generic[_T_Request, _T_Response]):
        ...
        *,
        ...
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        reader_future.add_done_callback(
                            functools.partial(
                                self.__serve_requests__when_reader_is_ready,
                                client_data=client_data,
                                selector_token=selector_token,
                                executor=executor,
                                task_exit_stack=task_exit_stack,
            ...
            if reader_future is not None:
                # Restarted via the done callback, directly execute it.
            ...
            self.__serve_request__run_client_function_in_executor(
            ...
            if not task_exit_stack.__exit__(type(exc), exc, exc.__traceback__):  # pragma: no branch
                raise
        finally:
            del self, reader_future, request_future  # Break reference cycle with request future on error.

    def __serve_request__run_client_function_in_executor(
        self,
        fn: Callable[[], _T_Return],
        executor: concurrent.futures.Executor,
        task_exit_stack: contextlib.ExitStack,
    ) -> concurrent.futures.Future[_T_Return]:
        try:
            handler_future = executor.submit(fn)
        except RuntimeError:
    ...
    def __serve_requests__when_reader_is_ready(
        self,
        reader_future: concurrent.futures.Future[float],
        /,
        *,
        client_data: _ClientData[_T_Request, _T_Response],
        selector_token: _SelectorToken,
        ...
        self.__serve_request__run_client_function_in_executor(handler, executor, task_exit_stack)
        ...
        *,
        ...
        with task_exit_stack.pop_all() as task_exit_stack:
            ...
            try:
                ...
                if (request_exc := request_future.exception(timeout=0)) is None:
                    request = request_future.result(timeout=0)
                ...
                client_data.transport_close_exit_stack.push(contextlib.closing(client_data.client))
                return
            finally:

...
from ..lowlevel.socket import SocketProxy, UnixSocketAddress, UNIXSocketAttribute, _peer_creds_or_lookup_error
...
class AsyncUnixStreamServer(
    ...
    async def __client_initializer(
    ...
    ) -> AsyncIterator[AsyncStreamClient[_T_Response]]:
        async with contextlib.AsyncExitStack() as client_exit_stack:
            ...
            self.logger.log(
                ...
                client.extra(UNIXClientAttribute.peer_name),
            )
    ...
    def __log_client_disconnection(logger: logging.Logger, level: int, client: _ConnectedClientAPI[_T_Response]) -> None:
        logger.log(level, "%s disconnected", client.extra(UNIXClientAttribute.peer_name))
...
class _ConnectedClientAPI(AsyncStreamClient[_T_Response]):
    ...
    def __init__(
        ...
        self.__peer_creds_cache = _unix_utils.LazyPeerCredsContainer()
        ...
        self.__extra_attributes_cache: Mapping[Any, Callable[[], Any]] = MappingProxyType(
            {
                **client.extra_attributes,
                ...
                UNIXClientAttribute.peer_credentials: _utils.make_callback(
                    self.__peer_creds_cache.get,
                    self.__proxy,
                    _peer_creds_or_lookup_error,
                ),
            }
        )

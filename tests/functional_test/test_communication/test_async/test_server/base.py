...
class BaseTestAsyncServer:
    ...
    @staticmethod
    def run_server(_bootstrap_server: _ServerBootstrapInfo) -> IEvent:
        return _bootstrap_server.is_up_event

    @pytest.fixture  # DO NOT SET autouse=True
    @staticmethod
    def server_task(_bootstrap_server: _ServerBootstrapInfo) -> Task[None]:
    ...
    async def test____serve_forever____shutdown_during_setup(
        ...
        async with backend.create_task_group() as tg:
            await tg.start(functools.partial(server_not_activated.serve_forever, is_up_event=event))
...
class BaseTestAsyncServerWithAsyncIO(BaseTestAsyncServer):
    @pytest.fixture(autouse=True)
    @staticmethod
    def disable_asyncio_warning_logs(caplog: pytest.LogCaptureFixture) -> None:
    ...
    async def _bootstrap_server(server: AbstractAsyncNetworkServer) -> AsyncIterator[_ServerBootstrapInfo]:
        ...
        event = server.backend().create_event()
        async with server.backend().create_task_group() as tg:
            task = await tg.start(serve_forever, server, event)
            yield _ServerBootstrapInfo(task, event)
            await server.shutdown()


@pytest.mark.feature_trio(async_test_auto_mark=True)
class BaseTestAsyncServerWithTrio(BaseTestAsyncServer):
    @pytest.fixture(autouse=True)
    @staticmethod
    def disable_trio_warning_logs(caplog: pytest.LogCaptureFixture) -> None:

...
class _BaseTestAsyncUDPNetworkServer(BaseTestAsyncServer):
    ...
    async def test____serve_forever____save_request_handler_context____server_shutdown(
        ...
        with endpoint.backend().timeout(1):
            while client_address not in request_handler.created_clients_map:
                await endpoint.backend().coro_yield()
...
class TestAsyncUDPNetworkServerWithAsyncIO(_BaseTestAsyncUDPNetworkServer, BaseTestAsyncServerWithAsyncIO):
    ...
    @pytest_asyncio.fixture
    @staticmethod
    async def server_address(run_server: IEvent, server: MyAsyncUDPServer) -> tuple[str, int]:
    ...
    async def client_factory(
        ...
        async with contextlib.AsyncExitStack() as stack:
            ...
            async def factory() -> AsyncDatagramSocket:
                endpoint = await AsyncDatagramSocket.open_udp_connection(
                    *server_address,
...
class TestAsyncUDPNetworkServerWithTrio(_BaseTestAsyncUDPNetworkServer, BaseTestAsyncServerWithTrio):
    @trio_fixture
    @staticmethod
    async def server_not_activated(
        request_handler: AsyncDatagramRequestHandler[str, str],

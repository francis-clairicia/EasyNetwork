...
class TestAsyncUnixDatagramClient(BaseTestClient):
    ...
    @pytest.fixture(autouse=True)
    @staticmethod
    def mock_os_stat(mocker: MockerFixture) -> MagicMock:
        mock_stat_result = mocker.NonCallableMagicMock(spec=os.stat_result)
        mock_stat_result.st_ino = 12345
        return mocker.patch("os.stat", autospec=True, return_value=mock_stat_result)

    @pytest.fixture(autouse=True)
    @staticmethod
    def mock_os_unlink(mocker: MockerFixture) -> MagicMock:
        return mocker.patch("os.unlink", autospec=True, return_value=None)
    ...
    def set_default_socket_mock_configuration(
        ...
        mock_backend.run_in_thread.side_effect = lambda f, *args, **options: f(*args)  # Needed to test unix socket deletion.
    ...
    async def test____dunder_init____with_remote_address____no_local_address____autobind_not_supported(
        ...
        # Act & Assert
        with pytest.raises(ValueError, match=r"^local_path parameter is required on this platform") as exc_info:
    ...
    async def test____aclose____await_socket_close(
        ...
        unix_socket_path = client_connected.get_local_name().as_pathname()
        ...
        if unix_socket_path is None:
            ...
            mock_os_unlink.assert_called_once_with(unix_socket_path)
    ...
    @pytest.mark.parametrize("global_local_address", ["/path/to/local_sock"], indirect=True)
    async def test____aclose____unix_socket_already_removed(
        self,
        client_connected: AsyncUnixDatagramClient[Any, Any],
        mock_os_stat: MagicMock,
        mock_os_unlink: MagicMock,
        ...
        mock_os_stat.side_effect = OSError(errno.ENOENT, os.strerror(errno.ENOENT))

        # Act
        await client_connected.aclose()

        # Assert
        mock_os_stat.assert_called_once_with(unix_socket_path)
        mock_os_unlink.assert_not_called()

    @pytest.mark.parametrize("global_local_address", ["/path/to/local_sock"], indirect=True)
    async def test____aclose____unix_socket_already_removed____race_condition(
        self,
        client_connected: AsyncUnixDatagramClient[Any, Any],
        mock_os_stat: MagicMock,
        mock_os_unlink: MagicMock,
        ...
        mock_os_unlink.side_effect = OSError(errno.ENOENT, os.strerror(errno.ENOENT))
    ...
    @pytest.mark.parametrize("global_local_address", ["/path/to/local_sock"], indirect=True)
    async def test____aclose____unix_socket_removed_then_reused(
        self,
        client_connected: AsyncUnixDatagramClient[Any, Any],
        mock_os_stat: MagicMock,
        mock_os_unlink: MagicMock,
        ...
        # Simulate file having a different inode.
        mock_os_stat.return_value.st_ino += 654
    ...
    @pytest.mark.parametrize("global_local_address", ["/path/to/local_sock"], indirect=True)
    async def test____aclose____unix_socket_cannot_be_removed____read_permission_denied(
        self,
        client_connected: AsyncUnixDatagramClient[Any, Any],
        mock_os_stat: MagicMock,
        mock_os_unlink: MagicMock,
        ...
        mock_os_stat.side_effect = OSError(errno.EPERM, os.strerror(errno.EPERM))

        # Act & Assert
        await client_connected.aclose()

    @pytest.mark.parametrize("global_local_address", ["/path/to/local_sock"], indirect=True)
    async def test____aclose____unix_socket_cannot_be_removed____write_permission_denied(
        self,
        client_connected: AsyncUnixDatagramClient[Any, Any],
        mock_os_stat: MagicMock,
        mock_os_unlink: MagicMock,
        ...
        mock_os_unlink.side_effect = OSError(errno.EPERM, os.strerror(errno.EPERM))

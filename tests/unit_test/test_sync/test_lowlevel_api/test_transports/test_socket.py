...
from easynetwork.lowlevel.socket import SocketAttribute, SocketProxy, TLSAttribute, UNIXSocketAttribute, socket_ucred
...
class TestSocketStreamTransport(MixinTestSocketSendMSG):
    ...
    def mock_stream_socket(
        ...
        match request.param:
            case "AF_INET":
                ...
                mock_stream_socket.getsockname.return_value = ("local_address", 11111)
                mock_stream_socket.getpeername.return_value = ("remote_address", 12345)
            case "AF_UNIX":
                mock_stream_socket = mock_unix_stream_socket_factory(socket_fileno)
                mock_stream_socket.getsockname.return_value = b"local_address"
                mock_stream_socket.getpeername.return_value = b"remote_address"
            case _:
        ...
        mock_stream_socket.sendmsg.side_effect = lambda buffers, *args: sum(map(len, map(memoryview, buffers)))
    ...
    def test____dunder_init____default(
        ...
        if mock_stream_socket.family == AF_INET:
            ...
            assert UNIXSocketAttribute.peercreds not in transport.extra_attributes
            assert transport.extra(UNIXSocketAttribute.peercreds, None) is None
            mock_get_peer_credentials.assert_not_called()
        else:
            assert transport.extra(SocketAttribute.sockname) == b"local_address"
            assert transport.extra(SocketAttribute.peername) == b"remote_address"

            assert UNIXSocketAttribute.peercreds in transport.extra_attributes
            assert transport.extra(UNIXSocketAttribute.peercreds) == fake_ucred
    ...
    @pytest.mark.parametrize("mock_stream_socket", ["AF_UNIX"], indirect=True)
    def test____extra_attributes____credentials_lookup_raises_OSError(
        ...
        # Arrange
        os_error = errno.EACCES
        mock_get_peer_credentials.side_effect = OSError(os_error, os.strerror(os_error))

        # Act & Assert
        with pytest.raises(TypedAttributeLookupError):
            transport.extra(UNIXSocketAttribute.peercreds)
        mock_get_peer_credentials.assert_called_once()
        assert transport.extra(UNIXSocketAttribute.peercreds, mocker.sentinel.default_value) is mocker.sentinel.default_value
...
class TestSocketDatagramTransport:
    ...
    def mock_datagram_socket(
        ...
        match request.param:
            case "AF_INET":
                ...
                mock_datagram_socket.getsockname.return_value = ("local_address", 11111)
                mock_datagram_socket.getpeername.return_value = ("remote_address", 12345)
            case "AF_UNIX":
                mock_datagram_socket = mock_unix_datagram_socket_factory(socket_fileno)
                mock_datagram_socket.getsockname.return_value = b"local_address"
                mock_datagram_socket.getpeername.return_value = b"remote_address"
            case _:
                pytest.fail(f"Invalid param: {request.param!r}")
    ...
    def test____dunder_init____default(
        ...
        if mock_datagram_socket.family == AF_INET:
            assert transport.extra(SocketAttribute.sockname) == ("local_address", 11111)
            assert transport.extra(SocketAttribute.peername) == ("remote_address", 12345)
        else:
            assert transport.extra(SocketAttribute.sockname) == b"local_address"

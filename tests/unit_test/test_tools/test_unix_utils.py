...
def test____is_unix_socket_family____AF_UNIX_defined() -> None:
    # Arrange
    import socket
...
@PlatformMarkers.runs_only_on_platform("win32", "Windows does not have AF_UNIX socket family")
def test____is_unix_socket_family____AF_UNIX_undefined() -> None:
    # Arrange
    import socket
...
if sys.platform != "win32":
    ...
    class TestSocketPeerCredentials:
        ...
        @pytest.fixture(autouse=True)
        @staticmethod
        def mock_find_library(mocker: MockerFixture) -> MagicMock:
            match sys.platform:
                case "darwin":
